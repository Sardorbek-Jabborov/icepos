<template>
  <div class="p-7 bg-white rounded-xl max-w-[586px]">
    <div class="flex justify-between pb-7 border-b border-gray-100">
      <p class="text-2xl font-bold text-blue-700"> Naxsulotlar</p>
      <button @click='close'>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th class="text-left">Miqdor</th>
              <th class="text-left">Orginal Narxi</th>
              <th class="text-left">Narxi</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(p, index) in props.object.products" :key="index">
              <td>{{ p.product.title }}</td>
              <td>{{ p.quantity }}</td>
              <td>{{ p.original_price }} UZS</td>
              <td>{{ p.price }} UZS</td>
            </tr>
          </tbody>
        </table>
